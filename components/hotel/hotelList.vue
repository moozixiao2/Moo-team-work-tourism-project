    ...
    >
      ...
      <div class="hotel-item-info">
        ...
        <div class="hotel-item-huanguan">
          {{item.alias}}
          <span v-if="item.hotellevel" :title="item.hotellevel.level + '星级'">
            <i
      ...
      <div class="hotel-item-price-wrap">
        <nuxt-link to="#">
          <el-row
            type="flex"
            justify="space-between"
            class="hotel-item-price"
            v-for="(item2, index2) in item.products"
            :key="index2"
          >
            <div class="hotel-item-price-title">{{item2.name}}</div>
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      this.pageIndex = val;
      // 重新获得数据
      this.pageData = this.hotelData.slice(
